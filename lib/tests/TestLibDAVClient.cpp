...
class TestLibDAVClient : public QObject {
 ...
 private slots:
  void initTestCase() {
    ...
    this->client =
        new WebDAV(Environment::get("LIBDAVCLIENT_WEBDAV_TEST_HOST"),
  ...
  void testWebDAVConnection() {
    WebDAVReply *reply = this->client->testConnection();
    connect(reply, &WebDAVReply::testConnectionResponse, [=](bool isSuccess) {
      qDebug() << "testConnection :" << isSuccess;
      QCoreApplication::exit(0);
      Q_ASSERT(true);
    });
    connect(reply, &WebDAVReply::error, [=](QNetworkReply::NetworkError err) {
  ...
  //  void testWebDAVCreateDir() {
  //    QString dirName =
  //    QDate::currentDate().toString(Qt::DateFormat::ISODate); WebDAVReply
  //    *reply = this->client->createDir(
  //        Environment::get("LIBDAVCLIENT_WEBDAV_TEST_PATH"), dirName);

  //    connect(reply, &WebDAVReply::createDirFinished, [=](QNetworkReply
  //    *reply) {
  //      if (!reply->error()) {
  //        qDebug() << "\nDir Created"
  //                 << "\nURL  :" << reply->url();
  ...
  //  void testWebDAVCopyDir() {
  //    QString dirName =
  //    QDate::currentDate().toString(Qt::DateFormat::ISODate); WebDAVReply
  //    *reply = this->client->copy(
  ...
  //  void testWebDAVMoveDir() {
  //    QString dirName =
  //    QDate::currentDate().toString(Qt::DateFormat::ISODate); WebDAVReply
  //    *reply = this->client->move(
  //        Environment::get("LIBDAVCLIENT_WEBDAV_TEST_PATH") + "/tttt.cpp",
  //        Environment::get("LIBDAVCLIENT_WEBDAV_TEST_PATH") + "/" + dirName +
  //            "/tttt.cpp",
  //        true);
  ...
  //  void testWebDAVRemoveDir() {
  //    QString dirName =
  //    QDate::currentDate().toString(Qt::DateFormat::ISODate); WebDAVReply
  //    *reply = this->client->remove(
  //        Environment::get("LIBDAVCLIENT_WEBDAV_TEST_PATH") + "/" + dirName);
  ...
  //  void testCardDAVCreateContact() {
  //    QString vCard =
  //        "BEGIN:VCARD\nVERSION:3.0\nN:Basak;Anupam;;;Basak\nADR;INTL;PARCEL;"
  //        "WORK:;;;;;;India\nEMAIL;INTERNET:anupam.basak27@gmail.com\nTEL;WORK:"
  //        "8981861008\nEND:VCARD";
  //    CardDAVReply *reply = this->cardDAVClient->createContact(this->uid,
  //    vCard); this->connect(reply, &CardDAVReply::createContactResponse,
  //                  [=](Contact *contact) {
  //                    QCoreApplication::exit(0);
  ...
  //  void testCardDAVListAllContacts() {
